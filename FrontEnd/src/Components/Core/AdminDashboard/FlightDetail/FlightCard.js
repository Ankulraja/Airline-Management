...
const FlightCard = (props) => {
  ...
  const departureTime = new Date(data.departureTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  const arrivalTime =  new Date(data.arrivalTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  ...
  let minutesDifference = Math.floor((timeDifferenceInMilliseconds % (1000 * 60 * 60)) / (1000 * 60));
 ...
 const modifyClickHandler =()=>{
  ...
  return (
    <div className="w-full h-full border border-gray-300 py-3 flex flex-row items-center my-3 rounded-lg
     shadow-[0px_3px_10px_rgb(128,128,128)] transition-all duration-300 hover:bg-blue-400">
      ...
      <div className="w-1/3 h-full flex justify-around items-center">
        ...
        <button onClick={modifyClickHandler} className="py-3 px-5 bg-blue-600 rounded-lg text-white font-bold">
            Modify
        </button>
      </div>
    </div>
